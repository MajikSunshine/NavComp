...
function navcomp:Help ()
	purchaseprint ("NavComp Commands")
	purchaseprint ("-  help - Prints this list")
	purchaseprint ("-  options - Displays Control screen")
	purchaseprint ("-  plot - Determines reasonable safe path between 2 sectors")
	purchaseprint ("-  save - Saves current navpath")
	purchaseprint ("-  load - Loads previously saved navpath")
	purchaseprint ("-  clear - Clears previously recorded storm data or hostile bot data")
	purchaseprint ("-  reset - Resets NavComp")
	purchaseprint ("-  evade - Toggles Evasion")
	purchaseprint ("-  autoreload - Activates autoreload for the current navpath")
end
...
function navcomp:SavePath (args)
	...
	if args and args [2] then
		...
		if args [5] then
			navcomp.data.activePath.autoReload = args [5]:lower () == "true"
		end
...
function navcomp:AutoReloadCurrentPath ()
	...
	navcomp.data.activePath.autoReload = true
end
