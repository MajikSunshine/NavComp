...
function navcomp:ClearActivePath ()
	...
	navcomp.data.activePath.autoReload = false
end
...
function navcomp:SyncSectorNotes ()
	local sysId, targetId, sectorId, notes, note, avoids
	if navcomp.data.avoidSectors then
		local avoids = navcomp.data:Clone (navcomp.data.avoidSectors)
	end
	navcomp.data.avoidSectors = {}
	navcomp.data.anchors = {}
	
	-- Update the anchor sector list
	for sysId=1, #SystemNames, 1 do
		if SystemNotes[sysId] then
			navcomp.note=SystemNotes[sysId]
			for x, y in pairs(SystemNotes[sysId]) do
				notes = {}
				if (x ~= "colors") and (x ~= "name") then
					notes[x] = y
					navcomp.note=notes
					...
					-- Check for manual avoids
						...
						if sectors then
							local v, h, sectorId, override
							navcomp.data.anchors [targetId] = {}
							for override, v, h in string.gmatch (sectors, "(%*?)(%a)%-?(%d%d?)") do
								override = override == "*"
								sectorId = navcomp:BuildSectorId (sysId, h, navcomp.data.sectors [v:upper ()])
								table.insert (navcomp.data.anchors [targetId], {s=sectorId, override=override})
							end
						end
					end
				end
	...
	-- Remove all changed systems from stress map due to changes in avoid sectors
	if avoids then
		for sectorId, _ in pairs (avoids) do
			if not navcomp.data.avoidSectors [sectorId] then
				-- Removed old avoid, clear system from cache
...
function navcomp:IsEncounteredStormSector (s1)
	if string.find (navcomp.data.stormSectorStr, "|" .. tostring (s1) .. "|") then
		return true
	else
		return false
	end
...
function navcomp:IsEncounteredBotSector (s1)
	if string.find (navcomp.data.botSectorStr, "|" .. tostring (s1) .. "|") then
		return true
	else
		return false
	end
...
function navcomp:IsAvoidSector (s1)
	return (navcomp.data.avoidSectors [s1] ~= nil)
end
...
function navcomp:IsWormholeSector (s1)
	return string.find (navcomp.data.whSectorStr, "|" .. tostring (s1) .. "|")
end
...
function navcomp:SortJumps (s, area)
